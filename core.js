...
5:{title:"Labirinto del cuore",desc:"Porta ❤️ fino all'uscita. Il percorso è stretto e qualche ostacolo si muove, ma è sempre possibile.",rules:[["🕹️","joystick a destra"],["22s","tempo limite"],["❤️","3 collisioni"]]},
...
function ruleSub(a,b){if(b.includes("vite")||b.includes("errori")||b.includes("collisioni")||b.includes("colpi"))return "Se finiscono, riparti da questo livello.";if(b.includes("joystick"))return "Pensato per il pollice destro.";if(b.includes("tempo"))return "Vai tranquilla, ma non troppo.";if(b.includes("round"))return "Ogni giro aggiunge qualcosa.";if(b.includes("raccogline"))return "Servono per caricare il colpo.";if(b.includes("COLPISCI"))return "Si illumina quando è pronto.";return "Regola semplice."}
...
function win(extra=""){active=false;cancelAnimationFrame(raf);const next=Math.min(7,level+1);if(level<6&&unlocked<next){unlocked=next;localStorage.setItem("amelieUnlocked",unlocked)}$("#resultStatus").textContent="LIVELLO SUPERATO";$("#resultTitle").textContent=level===6?"Ce l'hai fatta ❤️":"Brava, Amelie.";$("#resultText").textContent=extra||["Bacini raccolti. Tutto regolare.","Sei sopravvissuta ai bacini. Per ora.","Bel tempismo.","Memoria promossa.","Cuore arrivato a destinazione."][level-1];$("#resultBtn").textContent=level===6?"vedi il finale":"continua";$("#resultBtn").onclick=()=>{overlay.classList.remove("on");if(level===6)showFinal();else introLevel(level+1)};overlay.classList.add("on")}
function lose(reason=""){active=false;cancelAnimationFrame(raf);$("#resultStatus").textContent="RIPROVA";$("#resultTitle").textContent=taunts[Math.floor(Math.random()*taunts.length)];$("#resultText").textContent=reason||"Facciamone un'altra.";$("#resultBtn").textContent="riprova";$("#resultBtn").onclick=()=>{overlay.classList.remove("on");startLevel(level)};overlay.classList.add("on")}
